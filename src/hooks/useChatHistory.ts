...
import { isEmpty, wait } from "src/helpers";

function useChatHistory(initialValue: Array<ChatHistoryChunkInterface>) {
  ...
  const setValue = async (chunks: Array<ChatHistoryChunkInterface>) => {
    if (isEmpty(chunks)) {
      // reset chat
      ...
      for (const chunk of chunks) {
        ...
        if (user === "human") {
          ...
          setStoredValue((prevChunks) => {
            ...
            if (
              prevChunks.slice(-1)?.[0]?.texts?.slice(-1)?.[0] === "[TYPING]"
            ) {
          ...
          for (const text of texts) {
            ...
            setStoredValue((prevChunks) => {
              ...
              if (newChunk.texts.slice(-1)[0] !== "[TYPING]") {
                // if previous text is [TYPING] don't add a new one
                newChunk.texts = [...newChunk.texts, "[TYPING]"];
              }
            ...
            const typingTime = text.length * 40;
            ...
            setStoredValue((prevChunks) => {
              ...
              newChunk.texts[textsLength] = text;
              const newChunks = [...prevChunks];
